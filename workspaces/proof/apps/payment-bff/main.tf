...
module "payment_bff" {
  ...
  app_name    = "${each.key}-payment-bff"
  description = "Payment BFF - Express.js application running on Lambda (${each.key} environment)"
  ...
  api_gateway_description        = "API Gateway for Payment BFF (${each.key})"
